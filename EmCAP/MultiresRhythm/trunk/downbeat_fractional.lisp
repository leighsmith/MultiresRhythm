;;;; -*- Lisp -*-
;;;;
;;;; $Id: downbeat.lisp 5400 2008-12-29 21:03:50Z leigh $
;;;;
...
(defmethod amplitude-profile-downbeat-estimation ((ODF-fragment rhythm)
  ...
  (multiple-value-bind (shift-options shift-scores) 
      (match-ODF-meter meter tempo-in-bpm ODF-fragment :maximum-matches *max-amp-matches*) 
    ;; Convert shift positions into downbeats. Super dumb for now.
    (let* ((downbeats (.mod (./ shift-options (coerce beat-duration 'double-float)) beats-per-measure))
	   ...
	   (combined-probabilities (combine-probabilities (.mod (.round downbeats) beats-per-measure) 
...
(defun downbeat-estimate-rhythm (rhythm tempo-in-bpm meter beats-per-measure downbeat-estimator)
  ...
  (loop
     ...
     ;; We currently use the maximum amplitude matches for all downbeat estimates.
     ...
     ;; Perhaps we should pass in how many matches we want? Problem is if we can't fulfill that many.
     ;; collect locations in fractions of a beat and matching probabilities of each.
     for downbeat-probabilities = (funcall downbeat-estimator fragment-of-ODF meter beats-per-measure tempo-in-bpm bar-duration beat-duration)
     do (format t "tempo ~,2f bpm, search duration ~a search region ~a~%~%"
		tempo-in-bpm search-duration search-region)
     ;; probability weighted downbeat locations.
       (setf (.column downbeat-estimates measure-index) (.* downbeat-locations downbeat-probabilities))
...
(defun amass-evidence (estimates)
  ...
  (let ((location-evidence (/ (.sum estimates) (.array-dimension estimates 1))))
...
;;    (format t "amplitude estimates ~a~%" (.transpose amplitude-estimates))
    (format t "change in estimation over time ~a~%" (.partial-sum amplitude-estimates))
