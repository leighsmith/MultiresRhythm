function [ dataset_for_meter ] = make_quaero_dataset( annotations_directory, beat_file_extension )
%make_quaero_dataset Returns a cell vector of absolute pathnames of files in the corpus.
% $Id$
...
while (dataset_index <= maximum_examples && song_index <= maximum_examples)
     ...
     if (annotation_pathname(1) ~= '.' && ~annotation_files(song_index).isdir) % Remove all hidden files.
        ...
        beats_per_measure = read_ircam_annotation_timesignatures(beat_marker_filepath);
