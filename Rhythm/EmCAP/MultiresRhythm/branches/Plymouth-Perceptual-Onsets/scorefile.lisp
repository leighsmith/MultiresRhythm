...
(let ((note-tag 0))
  (defun write-note (file-handle part time duration key-number)
  ...
  instant. Increments the note-tag."
    ...
    ;;(format file-handle "amplitude: ~f;~%" velocities(timeIndex))
    (format file-handle "amplitude: ~f;~%" 1.0)
    (incf note-tag))
  ...
  (defun save-scorefile (filename onsets &key (duration 0.075) (description "")
    ...
    (with-open-file (f filename :direction :output :if-exists :supersede)
      (format f "// MusicKit scorefile ~a~%" filename)
      (format f "// Generated by save-score~%")
      (format f "info tempo: 60, title: \"~a\";~%" description)
      (loop
